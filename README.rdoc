...
Load time is logged separate because sometimes MongoMapper, not MongoDB, is the bottleneck.

(48.6ms) all query mes-geo.places {:ancestor_ids=>2347581, :place_type=>{"$in"=>["Town", "HistoricalTown"]}, :children_ids=>{:$not=>{:$size=>0}}, :sort=>[["name", 1]]}
(623.7ms) all load  mes-geo.places {:ancestor_ids=>2347581, :place_type=>{"$in"=>["Town", "HistoricalTown"]}, :children_ids=>{:$not=>{:$size=>0}}, :sort=>[["name", 1]]}
